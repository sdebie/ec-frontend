...
export function WishlistSummary({
  ...
  const addAllLabel =
    movedConfirmation != null
      ? `Moved ${movedConfirmation} ${movedConfirmation === 1 ? 'item' : 'items'} to cart ✓`
      : `Add all to cart (${n})`
    ...
    >
      ...
      <dl className="space-y-2 text-sm lg:mt-3 lg:border-t lg:border-(--sf-border) lg:pt-3">
        <div className="hidden items-center justify-between lg:flex">
          <dt className="text-(--sf-muted-text)">Saved</dt>
          <dd className="text-(--sf-text)">{savedLabel}</dd>
        </div>
        <div className="flex items-center justify-between">
          <dt className="text-(--sf-muted-text)">Ready to add</dt>
          <dd className="text-(--sf-text)">{n} of {items.length}</dd>
        </div>
      ...
      <div className="mt-3 flex flex-wrap items-center justify-center gap-x-4 gap-y-2 border-t border-(--sf-border) pt-3 lg:mt-4">
        <button
          type="button"
