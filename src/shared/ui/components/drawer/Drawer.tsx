  ...
  return (
    <div className={cn('flex flex-col space-y-1 p-6 border-b border-(--c-border) relative', className)}>
      ...
      <button
        type="button"
        onClick={onClose}
        className="absolute top-6 right-4 rounded-md p-1.5 opacity-70 transition-opacity hover:opacity-100 hover:bg-(--c-surface-hover) text-(--c-text) focus:outline-none focus:ring-2 focus:ring-(--c-ring) focus:ring-offset-1 focus:ring-offset-(--c-panel)"
      >
...
export function DrawerFooter({ className, children }: { className?: string; children: React.ReactNode }) {
  return (
    <div
      className={cn('mt-auto flex items-center justify-end space-x-2 p-6 border-t border-(--c-border) bg-(--c-panel)', className)}
    >
      {children}
    </div>
  )
