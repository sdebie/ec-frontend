...
const emptyList = {allCustomers: []}
const emptyCount = {customerCount: 0}
...
describe('useCustomers sorting', () => {
    ...
    it('omits sort from the list filterRequest when none is given', async () => {
        ...
        renderHook(() => useCustomers({page: 1, pageSize: 10}), {wrapper: createWrapper()})
    ...
    it('includes the given sort in the list filterRequest but not in the count', async () => {
        ...
        renderHook(
            () => useCustomers({page: 1, pageSize: 10, sort: [{field: 'user.createdAt', direction: 'DESC'}]}),
        ...
        expect(listVars.filterRequest).toEqual({
            filters: [],
            sort: [{field: 'user.createdAt', direction: 'DESC'}],
        })
