    ...
    <Card as="section" variant="bordered" data-testid="order-actions-panel">
      <Card.Body className="flex flex-col gap-4 p-5">
        <div className="flex items-center gap-3">
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-(--c-success)/15 text-(--c-success)">
            <Zap className="h-4 w-4" />
          </span>
          <h3 className="text-base font-semibold text-(--c-text)">Order Actions</h3>
        </div>
        ...
        {rest.length > 0 && (
          <>
            ...
            <DropdownMenu
              ...
              offset={4}
              trigger={
                <span className="flex h-(--c-control-h-md) w-full items-center justify-between rounded-(--c-radius) border border-(--c-border) px-4 text-sm font-medium text-(--c-text) transition-colors hover:border-(--c-accent) hover:text-(--c-accent)">
                  More actions
                ...
                meta.icon ? (
                  <DropdownItem key={meta.target} onClick={() => handleTransition(meta)}>
                    <span className="flex items-center gap-3">
                      <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-(--c-error)/15 text-(--c-error)">
                        <meta.icon className="h-4 w-4" />
                      </span>
                      <span>
                        <span className="block text-sm font-semibold text-(--c-text)">{meta.label}</span>
                        <span className="block text-xs text-(--c-text-muted)">{meta.description}</span>
                      </span>
                    </span>
                  </DropdownItem>
                ) : (
                  <DropdownItem key={meta.target} onClick={() => handleTransition(meta)} destructive={meta.destructive}>
                    {meta.label}
                  </DropdownItem>
                ),
              )}
