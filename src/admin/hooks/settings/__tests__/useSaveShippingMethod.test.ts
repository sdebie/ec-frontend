...
describe('useSaveShippingMethod', () => {
  ...
  it('calls adminGraphqlClient.request on mutate', async () => {
    vi.mocked(adminGraphqlClient.request).mockResolvedValue({
      saveShippingMethod: { id: '1', name: 'Standard', isActive: true, baseFee: 50, estimatedDays: '3-5 days' },
    })

    const { result } = renderHook(() => useSaveShippingMethod(), { wrapper: createWrapper() })

    act(() => {
      result.current.mutate({ id: '1', name: 'Standard', isActive: true, baseFee: 50, estimatedDays: '3-5 days' })
    })
  ...
  it('invalidates admin-shipping-methods cache on success', async () => {
    ...
    queryClient.setQueryData(['admin-shipping-methods'], [{ id: '1', name: 'Old', isActive: true, baseFee: 10, estimatedDays: '1-2 days' }])
  ...
  it('shows toast.error with duration:0 on ClientError', async () => {
    ...
    act(() => {
      result.current.mutate({ id: null, name: 'Express', isActive: true, baseFee: 100, estimatedDays: '1-2 days' })
    })
