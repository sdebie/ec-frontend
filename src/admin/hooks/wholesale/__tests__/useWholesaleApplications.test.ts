...
import {useWholesaleApplications} from '../useWholesaleApplications'
...
const emptyList = {allWholesaleApplications: []}
const emptyCount = {wholesaleApplicationCount: 0}

describe('useWholesaleApplications sorting', () => {
    ...
    it('includes the given sort in the list filterRequest but not in the count', async () => {
        ...
        expect(listVars.filterRequest).toEqual({filters: [], sort: [{field: 'createdAt', direction: 'DESC'}]})
