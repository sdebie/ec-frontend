    ...
    return (
        <section className="bg-white p-6 rounded-2xl shadow-sm border">
            <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
                <span
                    className="bg-blue-600 text-white w-6 h-6 rounded-full text-xs flex items-center justify-center">1</span>
                ...
                <div className="mt-2 text-xs">
                    ...
                    {emailValid && (
                        <div>
                            {isAuthenticated ? (
                                <span
                                    className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-green-50 text-green-700 border border-green-100">
                  Welcome back{customer?.firstName ? `, ${customer.firstName}` : ''}! ({email})
                                ...
                                <>
                                    ...
                                    {lookupState === 'found' && customer && customer.shopperType?.toUpperCase() === CustomerType.REGISTERED.toUpperCase() && (
                                        <span
                                            className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-blue-50 text-blue-700 border border-blue-100">
                      ...
                      No account found — continuing as guest
                                    ...
                                    {lookupState === 'error' && (
                                        <span
                                            className="text-yellow-700 bg-yellow-50 border border-yellow-200 px-2 py-1 rounded-md">Could not check account right now.</span>
                                    )}
                                </>
                            )}
                        </div>
