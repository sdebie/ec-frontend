...
import type {OrderItemsData} from '@/pages/shop/default/cart/types.ts';
...
const OrderSummaryItem: React.FC<Props> = ({item, index}) => {
    ...
    const attrs = parseAttributesJson(item?.variant?.attributesJson);
    const imageUrl = item?.variant?.images?.[0]?.imageUrl;
    ...
    return (
        <li className="flex gap-4 border-b border-gray-100 pb-4 last:border-b-0 last:pb-0">
            <div className="h-16 w-16 shrink-0 overflow-hidden rounded-md border border-gray-200 bg-white">
                {imageUrl ? (
                    <img src={imageUrl} alt={item?.variant?.product?.name || 'Product'}
            ...
            <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium text-gray-900">{item?.variant?.product?.name || `Product ${index + 1}`}</p>
