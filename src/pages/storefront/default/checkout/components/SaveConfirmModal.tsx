        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            <div className="relative z-10 w-full max-w-md rounded-2xl border border-(--sf-border) bg-(--sf-panel) p-6 shadow-xl">
                ...
                <div className="mt-4 space-y-2 text-sm">
                    ...
                    {needsShippingAddress && (
                        <>
                            <div className="flex items-start gap-2"><span className="w-20 text-(--sf-muted-text)">Street</span><span className="font-medium text-(--sf-text)">{address.street || '-'}</span></div>
                            <div className="flex items-start gap-2"><span className="w-20 text-(--sf-muted-text)">City</span><span className="font-medium text-(--sf-text)">{address.city || '-'}</span></div>
                            <div className="flex items-start gap-2"><span className="w-20 text-(--sf-muted-text)">Postal</span><span className="font-medium text-(--sf-text)">{address.postalCode || '-'}</span></div>
                            <div className="flex items-start gap-2"><span className="w-20 text-(--sf-muted-text)">Province</span><span className="font-medium text-(--sf-text)">{address.province || '-'}</span></div>
                        </>
                    )}
                </div>
