...
import {uploadProductPriceCsv} from "@/services/rest/admin/ProductPriceService.rest.ts";
...
const ProductBulkUpload = () => {
    ...
    const handleUpload = async () => {
        ...
        try {
            await uploadProductPriceCsv(file);

            setIsDialogOpen(false);
            navigate("/admin/imports/products/price/list");
        } catch (error) {
    ...
    return (
        <Dialog open={isDialogOpen} onClose={handleClose} size="lg">
            <DialogHeader
                title="Bulk Product Price Update"
                description="Upload a CSV file to update prices in bulk."
            />
